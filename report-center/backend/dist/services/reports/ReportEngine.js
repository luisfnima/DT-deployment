...
const CRMService_1 = require("../crm/CRMService");
const HTMLRenderer_1 = require("../../renderers/HTMLRenderer");
const EvolutionService_1 = require("../whatsapp/EvolutionService");
const PDTInstalarReport_1 = require("./PDTInstalarReport");
const SeguimientoBOReport_1 = require("./SeguimientoBOReport");
const InstaladasHoyReport_1 = require("./InstaladasHoyReport");
...
class ReportEngine {
    constructor() {
        ...
        this.modules = {};
        this.crmService = new CRMService_1.CRMService();
        this.htmlRenderer = new HTMLRenderer_1.HTMLRenderer();
        this.evolutionService = new EvolutionService_1.EvolutionService();
        // Register current modules
        this.registerModule('1', new PDTInstalarReport_1.PDTInstalarReport());
        this.registerModule('2', new SeguimientoBOReport_1.SeguimientoBOReport());
        this.registerModule('3', new InstaladasHoyReport_1.InstaladasHoyReport());
    ...
    async generatePreview(reportId) {
        const report = ReportRepository_1.ReportRepository.getReportById(reportId);
        if (!report) {
            throw new Error(`Reporte ID ${reportId} no encontrado.`);
        }
        if (report.isScreenshot || report.template === 'screenshot') {
            ReportRepository_1.ReportRepository.addLog('Scheduler', `📸 Generando vista previa de capturas para "${report.name}"...`, 'info', report.id);
            let buffers = [];
            let screenshotError = '';
            try {
     ...
     */
    async executeReport(reportId, executionType = 'automatic') {
        const report = ReportRepository_1.ReportRepository.getReportById(reportId);
        if (!report) {
            ReportRepository_1.ReportRepository.addLog('System', `Reporte ID ${reportId} no encontrado.`, 'error');
            return false;
        }
        const startTime = Date.now();
        const isTest = executionType === 'test';
        const tagLabel = isTest ? '[PRUEBA]' : '';
        ReportRepository_1.ReportRepository.addLog('Scheduler', `${tagLabel} 🟢 Iniciando ejecución de reporte: "${report.name}" (${executionType})`, 'info', report.id);
        try {
            let content = '';
            let imageBuffer;
            let excelBuffer;
            ...
            if (report.isScreenshot || report.template === 'screenshot') {
                ...
                ReportRepository_1.ReportRepository.addLog('CRM', `${tagLabel} 🟡 Consultando y procesando datos en módulo "${report.name}"...`, 'info', report.id);
                reportData = await module.run(this.crmService);
                // 3. Render HTML
                ReportRepository_1.ReportRepository.addLog('Renderer', `${tagLabel} 🔵 Generando reporte en formato HTML...`, 'info', report.id);
                content = this.htmlRenderer.render(reportData);
                // 4. Render to Attachment (Excel for report 6, PNG image for others)
                if (report.id === '6') {
                    ReportRepository_1.ReportRepository.addLog('Renderer', `${tagLabel} 📊 Generando archivo Excel (.xlsx) con exceljs...`, 'info', report.id);
                    ...
                    ReportRepository_1.ReportRepository.addLog('Renderer', `${tagLabel} 📸 Convirtiendo HTML a imagen PNG usando Playwright...`, 'info', report.id);
                    const imageRenderer = new ImageRenderer_1.ImageRenderer();
                    imageBuffer = await imageRenderer.renderHtmlToPng(content);
                }
            }
            // 5. Get recipients associated to this report
            const allRecipients = ReportRepository_1.ReportRepository.getRecipients();
            ...
            for (const recipient of reportRecipients) {
                ...
                if (executionType === 'automatic' && recipient.allowedWindow) {
                    ...
                    if (hourStr < start || hourStr > end) {
                        ReportRepository_1.ReportRepository.addLog('Sender', `⚠️ Envío omitido para "${recipient.name}": Hora actual (${hourStr}) fuera del rango permitido (${start} - ${end})`, 'warning', report.id);
                        continue;
                    }
                }
                ReportRepository_1.ReportRepository.addLog('Sender', `${tagLabel} 🟣 Despachando a "${recipient.name}" via ${recipient.channel.toUpperCase()} (${recipient.value})...`, 'info', report.id);
                try {
                    if (recipient.channel === 'whatsapp' || recipient.channel === 'whatsapp_group') {
                        ...
                        ReportRepository_1.ReportRepository.updateRecipient(recipient.id, { lastDeliveryAt: new Date().toISOString() });
                        sendCount++;
                        recipientListString += `${recipient.name} (${recipient.value}), `;
                    }
                    else {
                        ReportRepository_1.ReportRepository.addLog('Sender', `⚠️ Canal "${recipient.channel}" preparado pero no implementado en este MVP.`, 'info', report.id);
                    }
                }
                catch (sendErr) {
                    ReportRepository_1.ReportRepository.addLog('Sender', `❌ Fallo al enviar a "${recipient.name}": ${sendErr.message}`, 'error', report.id);
                }
            }
            if (sendCount === 0) {
            ...
            ReportRepository_1.ReportRepository.addLog('Scheduler', `✅ Finalizado: Reporte "${report.name}" enviado con éxito a ${sendCount} destinatarios en ${durationMs}ms.`, 'success', report.id);
            // Record counts
            let recordsCount = 0;
            if (report.isScreenshot) {
                ...
                recordsCount = Object.values(reportData.porSupervisor).reduce((acc, vs) => acc + (vs?.length || 0), 0);
            }
            else if (reportData.isSeguimientoBO && reportData.porSupervisor) {
                recordsCount = Object.values(reportData.porSupervisor).reduce((acc, val) => acc + (val?.totalVentas || 0), 0);
            }
            ...
            // Save history record
            ReportRepository_1.ReportRepository.addHistoryRecord({
                reportId: report.id,
            ...
            if (!isTest) {
                ReportRepository_1.ReportRepository.updateReport(report.id, {
            ...
            ReportRepository_1.ReportRepository.addLog('System', `❌ Error ejecutando reporte "${report.name}": ${errorMsg}`, 'error', report.id);
            ReportRepository_1.ReportRepository.addHistoryRecord({
                reportId: report.id,
                reportName: report.name,
                status: 'failed',
     ...
     */
    async performLogin(page, loginUrl, username, password, uSel, pSel, sSel, reportId) {
        if (!loginUrl || loginUrl.trim() === '')
            return;
        ReportRepository_1.ReportRepository.addLog('Scheduler', `🔑 Navegando a página de login: ${loginUrl}`, 'info', reportId);
        ...
        if (!page.url().includes('/login')) {
            ReportRepository_1.ReportRepository.addLog('Scheduler', `ℹ️ Redirigido automáticamente fuera de /login. Sesión activa.`, 'info', reportId);
            return;
        }
        if (username && password) {
            ...
            try {
                ...
                ReportRepository_1.ReportRepository.addLog('Scheduler', `⚠️ Advertencia durante el login: ${loginErr.message}`, 'warning', reportId);
            }
        }
    }
    /**
        ...
        try {
            ...
            if (report.screenshotTargets && report.screenshotTargets.length > 0) {
                ...
                for (let i = 0; i < validTargets.length; i++) {
                    ...
                    ReportRepository_1.ReportRepository.addLog('Scheduler', `📸 Procesando captura ${i + 1}/${validTargets.length}: ${url}`, 'info', report.id);
            ...
            for (let i = 0; i < cleanUrls.length; i++) {
                ...
                ReportRepository_1.ReportRepository.addLog('Scheduler', `📸 Tomando captura ${i + 1}/${cleanUrls.length}: ${url}`, 'info', report.id);
                try {
                    const currentUrl = page.url();
                    const cleanTarget = url.replace(/\/$/, '');
                    const cleanCurrent = currentUrl.replace(/\/$/, '');
