...
exports.CanceladasHoyReport = void 0;
const helpers_1 = require("./helpers");
class CanceladasHoyReport {
    async run(crmService) {
        const rawSales = await crmService.fetchData('Canceladas Hoy');
        ...
        const filtered = rawSales.filter((v) => {
            if ((v.status || '').toUpperCase() !== 'CANCELADO')
                return false;
        ...
        return {
            reportName: 'Canceladas Hoy',
            subTitle: 'Telefonía Izaguirre',
            timestamp: new Date().toISOString(),
            isGrouped: true,
            metrics: {
                'Total Canceladas': sorted.length,
...
exports.CanceladasHoyReport = CanceladasHoyReport;
