...
class HTMLRenderer {
    ...
    render(data) {
      ...
      <div style="font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 900px; margin: 0 auto; padding: 24px; background-color: #ffffff; border: 1px solid #e4e4e7; border-radius: 16px; color: #18181b; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);">
        ...
        <!-- Header Banner -->
        <div style="background: linear-gradient(135deg, ${theme_1.themeConfig.colors.primary} 0%, #b71c1c 100%); padding: 24px; border-radius: 12px; margin-bottom: 24px; color: #ffffff;">
    ...
    `;
      ...
      `;
        ...
        if (isSeguimientoBO && porSupervisor) {
            Object.entries(porSupervisor).forEach(([supervisor, data]) => {
                html += `
          ...
          `;
                data.registros.forEach((v, index) => {
              ...
              <tr style="background-color: ${rowBg}; border-bottom: 1px solid #f4f4f5;">
                ...
                ` : `
                  <td style="padding: 10px 12px; font-family: monospace; font-weight: 600;">${v.sale_date ? (0, helpers_1.soloHora)(v.sale_date) : '-'}</td>
                `}
        ...
        `;
            ...
            Object.entries(porSupervisor).forEach(([supervisor, vs]) => {
                html += `
          <div style="margin-bottom: 30px; border: 1px solid #e4e4e7; border-radius: 10px; overflow: hidden;">
            <!-- Supervisor Badge Header -->
            <div style="background-color: #f4f4f5; border-bottom: 1px solid #e4e4e7; padding: 12px 16px; display: flex; justify-content: space-between; align-items: center;">
              <span style="font-weight: 700; font-size: 13px; color: #18181b;">👤 ${supervisor}</span>
              <span style="background-color: ${theme_1.themeConfig.colors.primary}; color: #ffffff; font-size: 11px; font-weight: 700; padding: 2px 8px; border-radius: 9999px;">
        ...
        `;
            ...
            <tr style="background-color: ${rowBg}; border-bottom: 1px solid #f4f4f5;">
              ...
              <td style="padding: 10px 14px; font-weight: 600; color: #b62525;">${v.cabecera_f_instalacion ? (0, helpers_1.soloDia)(v.cabecera_f_instalacion) : '-'}</td>
